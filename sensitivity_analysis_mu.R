...
cos_embedding_sens_anal_mu<-function(A, G, p, mu, mu_nosy, lambda, gamma, gamma_nosy, ep, iter_max){
...
tms = 10
ranosy_mu <- seq(from = 0, to = 5, by = 1)
# xybaseline_mu <- numeric(length(ranosy_mu))
# xynosy_mu <- numeric(length(ranosy_mu))
# xyabs_mu <- numeric(length(ranosy_mu))

mu_nosy0 <- matrix(0,tms,length(ranosy_mu))
mu0 <- matrix(0,tms,length(ranosy_mu))
mu_nosy0_hat <- matrix(0,tms,length(ranosy_mu))
mu0_hat <- matrix(0,tms,length(ranosy_mu))
baseline_mu = matrix(0,tms,length(ranosy_mu))
nosy_mu = matrix(0,tms,length(ranosy_mu))


for (x in 1:length(ranosy_mu)){
# for (y in 1:length(ranosy)){
  ...
  for (t in 1:tms)
  {
   ...
   mu0[t,x]<-log(pho/(1-pho)) #初始mu值
   mu_nosy0[t,x]<-log(pho/(1-pho))+rnorm(1,0,ranosy[x]) #初始mu值+ 扰动eta
   ...
   sensitivity_analysis <- cos_embedding_sens_anal_mu(A=adj_matrix, G=K, p=K, mu, mu_nosy, lambda, gamma, gamma_nosy, ep, iter_max)
   
   baseline_mu[t,x] <- miscluster(sensitivity_analysis$group,block_labels,2)$misrate #初始mu分组结果
   nosy_mu[t,x] <- miscluster(sensitivity_analysis$group_nosy,block_labels,2)$misrate #初始mu+eta分组结果
   
   mu0_hat[t,x] <- sensitivity_analysis$mu_hat #初始mu得到的估计mu
   mu_nosy0_hat[t,x] <- sensitivity_analysis$mu_hat_nosy #初始mu+eta得到的估计mu_nosy0_hat
  ...
  # xybaseline_mu[x] <- mean(baseline_mu[,x])
  # xynosy_mu[x] <- mean(nosy_mu[,x])
  # xyabs_mu[x] <- sum(abs(nosy_mu[,x]-baseline_mu[,x]))
# }
}


write.csv(mu0,file = "C:/Users/lenovo/Desktop/2026revise/mu0.csv")
write.csv(mu_nosy0,file = "C:/Users/lenovo/Desktop/2026revise/mu_nosy0.csv")
write.csv(baseline_mu,file = "C:/Users/lenovo/Desktop/2026revise/baseline_mu.csv")
write.csv(nosy_mu,file = "C:/Users/lenovo/Desktop/2026revise/nosy_mu.csv")
write.csv(mu0_hat,file = "C:/Users/lenovo/Desktop/2026revise/mu0_hat.csv")
write.csv(mu_nosy0_hat,file = "C:/Users/lenovo/Desktop/2026revise/mu_nosy0_hat.csv")
